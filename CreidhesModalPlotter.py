...
def PlotModalFile(filename,datsize):
...
#	allampYre = np.zeros((ny,nx))
	...
	allampX[:,:] = np.reshape(data[:,4],(ny,nx))
	allphiX[:,:] = np.reshape(data[:,5],(ny,nx))
	allampY[:,:] = np.reshape(data[:,6],(ny,nx))
	allphiY[:,:] = np.reshape(data[:,7],(ny,nx))
